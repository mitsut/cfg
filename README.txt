...
は必要はありません）。
コンフィギュレータの構築にはBoost C++ Librariesが必要です。Boost C++
Librariesは、下記URLから入手することができます。

http://www.boost.org/

Cygwinの場合、setup.exeからBoost C++ Libireisをインストールすることが可
能です。Linuxの場合、ディストリビューションによっては、OSとともにインス
トールが可能であったり、yumやapt-getコマンド等でインストールできる可能
性があります。Macの場合、MacPortでインストールすることが可能です。
